...
/*		spatialData[0] = data[i]->acceleration[0]*1000000;
		spatialData[1] = data[i]->acceleration[1]*1000000;
		spatialData[2] = data[i]->acceleration[2]*1000000;*/

		spatialData2[0] = data[i]->angularRate[0];
		spatialData2[1] = data[i]->angularRate[0];
		spatialData2[2] = data[i]->angularRate[0];

		/*spatialData3[0] = data[i]->acceleration[0];
		spatialData3[1] = data[i]->acceleration[1];
		spatialData3[2] = data[i]->acceleration[2];*/
...
void drawSpatial2(IplImage *frame)
{
	...
	CvPoint pt1 = cvPoint(x00, (int)2*y0);
	CvPoint pt2 = cvPoint(x00, (int)7*y0);
	cvLine(frame, pt1, pt2, green, 1, 8);

	cvCircle(frame, cvPoint(frame->width / 2, frame->height / 2), 60 + (int)vibration, red, .1, CV_AA, 0);
	cvCircle(frame, cvPoint(frame->width / 2, frame->height / 2), 91, green, 1, CV_AA, 0);

/*	char text[255];
	//sprintf(text, "Score %d \n comprimento: %d \n altura: %d", (int)vertical, (int)7 * x00 / 2, (int)size*9);
	sprintf(text, "variavel vibr: %f", vibration);
	CvFont font;
	double hScale = 1.0;
	double vScale = 1.0;
	int    lineWidth = 1;
	cvInitFont(&font, CV_FONT_HERSHEY_SIMPLEX | CV_FONT_ITALIC, hScale, vScale, 0, lineWidth);
	cvPutText(frame, text, cvPoint(0, 200), &font, black);
*/
...
int main()
{
	...
	// Create window
	cvNamedWindow("Camera_Output", CV_WINDOW_NORMAL);
